...
export const onRequestGet: PagesFunction<Env> = async ({ request, env, params }) =>
  handleRequest(async () => {
    ...
    const autosave = await getLatestArticleAutosave(db, readId(params));

    return json({ autosave });
  });
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) =>
  handleRequest(async () => {
    const db = requireDb(env);
    await requireAdmin(request, env, db);
    const autosave = await createArticleAutosave(
      db,
      readId(params),
      readCmsArticleInput(await readJsonBody(request)),
    );

    return json({ autosave }, { status: 201 });
  });
